...
class App():
    ...
    def build_third_keypad_layer(self):
        ...
        self.f1_button = self.create_button("f1_button", self.frame, FIRST_FLOOR_LABEL, self.set_elevator_floor(FIRST_FLOOR), 1, 12)
        self.f2_button = self.create_button("f2_button", self.frame, SECOND_FLOOR_LABEL, self.set_elevator_floor(SECOND_FLOOR_LABEL), 3, 12)
        self.f3_button = self.create_button("f3_button", self.frame, THIRD_FLOOR_LABEL, self.set_elevator_floor(THIRD_FLOOR_LABEL), 5, 12)


    def build_fourth_keypad_layer(self):
        """ Makes buttons for fourth layer of the keypad."""

        self.f4_button = self.create_button("f4_button", self.frame, FOURTH_FLOOR_LABEL, self.set_elevator_floor(FOURTH_FLOOR_LABEL), 1, 9)
        self.f5_button = self.create_button("f5_button", self.frame, FITH_FLOOR_LABEL, self.set_elevator_floor(FITH_FLOOR_LABEL), 3, 9)
        self.f6_button = self.create_button("f6_button", self.frame, SIXTH_FLOOR_LABEL, self.set_elevator_floor(SIXTH_FLOOR_LABEL), 5, 9)


    def build_fith_keypad_layer(self):
        """ Makes buttons for fith layer of the keypad."""

        self.f7_button = self.create_button("f7_button", self.frame, SEVENTH_FLOOR_LABEL, self.set_elevator_floor(SEVENTH_FLOOR_LABEL), 1, 6)
        self.f8_button = self.create_button("f8_button", self.frame, EIGHTH_FLOOR_LABEL, self.set_elevator_floor(EIGHTH_FLOOR_LABEL), 3, 6)
        self.f9_button = self.create_button("f9_button", self.frame, NEIGH_FLOOR_LABEL, self.set_elevator_floor(NEIGH_FLOOR_LABEL), 5, 6)


    def build_sixth_keypad_layer(self):
        """ Makes buttons for sixth layer of the keypad."""

        self.f10_button = self.create_button("f10_button", self.frame, TENTH_FLOOR_LABEL, self.set_elevator_floor(TENTH_FLOOR_LABEL), 1, 3)
        self.f11_button = self.create_button("f11_button", self.frame, ELEVENTH_FLOOR_LABEL, self.set_elevator_floor(ELEVENTH_FLOOR_LABEL), 3, 3)
        self.f12_button = self.create_button("f12_button", self.frame, TWELFTH_FLOOR_LABEL, self.set_elevator_floor(TWELFTH_FLOOR_LABEL), 5, 3)
    ...
    def display_elevator_attr(self):        
        ...
        self.current_temp_label = self.create_label(CURRENT_TEMPERATURE, LABEL_SIZE, column=1, row= 1)  # current temp
        self.current_weight_label = self.create_label(CURRENT_WEIGHT, LABEL_SIZE, column=1, row= 2) # current weight
